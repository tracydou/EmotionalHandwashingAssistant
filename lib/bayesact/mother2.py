...
class Mother(Agent):
    def __init__(self,*args,**kwargs):
        ...
        #distributions for action 0 = "soothe" (give morphine to) and 1 = "torment" (not give morphine to)
        #self.ppx=[[[0.1,0.7,0.2],[0.05,0.75,0.2],[0.0,0.0,1.0]],
        #          [[0.99,0.0,0.01],[0.85,0.14,0.01],[0.0,0.0,1.0]]]
        self.ppx=[[[0.0,p_val,(1-p_val)],[0.0,p_val,(1-p_val)],[0.0,0.0,1.0]],
                  [[q_val,0.0,(1-q_val)],[q_val,0.0,(1-q_val)],[0.0,0.0,1.0]]]
    ...
    def evalSampleFvar(self,fvars,tdyn,state,ivar,ldenom,turn,observ):
        weight=0.0
        if (not observ==[]):   #also works on agent turn for this example
    ...
    def oracle(self,state,rollout=False,samplenum=None):
        ...
        if state.get_turn()=="agent":
            if state.x[1]==2:
                #son is dead, so must take the worst action - a hack to make it always get the big deflection
                paab = 1
                a = action_kill
            #switch between the two first sample returned is the default (ACT) action
            elif samplenum%2==1:
                paab = 1            #not give morphine to
                a = action_not_morphine
            else:
    ...
    #this should work by comparing only the first of the affective actions - the second is always "kill"
    def hasAction(self,actionSet,action,actres):
        for a1 in actionSet:
            if a1[1]==action[1] and math.sqrt(raw_dist(action[0][0][0],a1[0][0][0])) < actres:       
                return True
        return False
    ...
    def actionDist(self,a1,a2):
        if a1[1]==a2[1]:
            return math.sqrt(raw_dist(a1[0][0][0],a2[0][0][0]))
        else:
            return -1
...
def ask_client(fbeh,sugg_act='',epaact=[]):
    ...
    sstnc+="- pick an action (0: give, 1: not give)\n"
...
while not done:
    ...
    if learn_paab==0:   #give morphine
        aact = "give morphine"
    else:
        aact = "not give morphine"
    print "suggested action for the agent is :",learn_paab,"\n  closest label is: ",aact

    learn_aab=ask_client(fbehaviours_agent)
    ...
    if curr_painlevel==2:  #dead
        lean_observ=action_kill[0][1]
    elif learn_paab==0:   #give morphine
        learn_observ=action_morphine[0][0]
    else:  #not give morphine
        learn_observ=action_not_morphine[0][0]

    if learn_paab==0:
        learn_aab=action_morphine
    else:
        learn_aab=action_not_morphine
