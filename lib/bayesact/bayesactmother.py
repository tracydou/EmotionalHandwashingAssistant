...
bayesactmother is for use with the mother.py example - it changes sampleFvar so it samples 
...
p_val=0.6   #probability that giving morphine will not cause death  (1-p_val: prob that morphine causes death)
q_val=0.95  #probability that not giving morphine will not cause death

action_not_morphine = (([-3.2,1.4,0.94],[-4.05, 1.08, 1.35]),(q_val,1-q_val)) #torment,kill
action_morphine = (([2.89, 1.56, -0.77] ,[-4.05, 1.08, 1.35]),(p_val,1-p_val))  #soothe,kill
action_kill     = (([2.89, 1.56, -0.77] ,[-4.05, 1.08, 1.35]),(0.0,1.0))  #kill,kill
action_soothe     = (([2.89, 1.56, -0.77] ,[-4.05, 1.08, 1.35]),(1.0,0.0))  #
action_torment = (([-3.2,1.4,0.94],[-4.05, 1.08, 1.35]),(1.0,0.0)) #torment,kill
...
def sampleFvar(fvars,tvars,H,C,isiga,isigf,tau,f,turn="agent",aab=[],observ=[]):
    ...
    #aab is a tuple with aab[0] = (aab[0][0],aab[0][1],...) two or more actions
    #and aab[1] = (p0,p1,..) multinomial probability
    if turn=="agent" and not aab==[]:
        acttotake = list(NP.random.multinomial(1,aab[1])).index(1)
        f=NP.concatenate((f[0:3],aab[0][acttotake],f[6:9]))
...
class Agent(object):
    ...
    def propagateSample(self,state,action):
        ...
        #first, draw an action to take  - either (soothe/torment) or kill based on if the action is to give morphine/not give morphine
        acttotake = list(NP.random.multinomial(1,aab[1])).index(1)

        #next, pick the corresponding behaviour and generate an observation of fb from that
        observ = map(lambda fv: NP.random.normal(fv,self.gamma_value_pomcp), aab[0][acttotake])
